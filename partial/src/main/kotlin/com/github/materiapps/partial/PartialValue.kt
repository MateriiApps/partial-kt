...
@Serializable(PartialValueSerializer::class)
public sealed interface PartialValue<out T> {
...
internal class PartialValueSerializer<T>(
    ...
    override fun serialize(encoder: Encoder, value: PartialValue<T?>) {
        if (value is PartialValue.Value) {
            if (value.value != null) {
